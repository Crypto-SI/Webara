...
import { requireAdminProfile, requireAuthenticatedUser } from '@/lib/auth-server';
...
function summarizeText(text?: string | null, fallback = 'No summary provided yet.') {
  if (!text) {
    return fallback;
  }
...
function sanitizeSuggestions(value: QuoteRow['ai_suggestions']): string[] {
  ...
  if (
    value &&
    typeof value === 'object' &&
    'suggestions' in (value as Record<string, unknown>)
  ) {
